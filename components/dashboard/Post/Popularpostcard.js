...
export default function PopularPostCard({ post }) {
    ...
    return (
        <Link href={`/blog/${post.slug}`} className="group flex gap-3 items-start hover:bg-slate-50 p-2 rounded-xl transition-colors">
            {/* Thumbnail */}
            <div className="relative flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-gradient-to-r from-[#f97316] to-[#fb8a3c]">
                {imgSrc ? (
            ...
            <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-slate-800 group-hover:text-[#f97316] transition-colors line-clamp-2 leading-snug">
                    {post.title}
                </h4>
                ...
                {post.category && (
                    <span className="inline-block mt-1 text-xs text-[#f97316] font-medium">{post.category.name}</span>
                )}
            </div>
        </Link>
    );
