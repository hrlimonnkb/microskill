...
// ✅ SSR থেকে initialCourse এবং initialRelatedCourses props হিসেবে আসবে
export default function CoursePageClient({ course: initialCourse, relatedCourses: initialRelatedCourses, slug }) {
    ...
    // ✅ SSR থেকে আসা data দিয়ে initialize
    const [course, setCourse] = useState(initialCourse);
    const [loading, setLoading] = useState(false); // SSR করেছে তাই false
    const [error, setError] = useState(null);
    const [introVideoUrl, setIntroVideoUrl] = useState('');
    const [openSections, setOpenSections] = useState({});
    const [isEnrolled, setIsEnrolled] = useState(false);
    const [enrollmentLoading, setEnrollmentLoading] = useState(false);
    const [relatedCourses, setRelatedCourses] = useState(initialRelatedCourses || []);

    useEffect(() => {
        // SSR DEBUG - পরে মুছে দিও
        console.log('📄 CoursePageClient mounted');
        console.log('🖥️ SSR data আছে:', initialCourse ? '✅ হ্যাঁ' : '❌ নেই');

        if (initialCourse) {
            // SSR থেকে data পাওয়া গেছে, শুধু UI setup করো
            const parsedIntroUrl = parseIntroVideo(initialCourse.introVideo);
            if (parsedIntroUrl) {
                setIntroVideoUrl(parsedIntroUrl);
                console.log('Intro Video URL parsed:', parsedIntroUrl);
            }
            if (initialCourse.sections?.[0]) {
                setOpenSections({ [initialCourse.sections[0].id]: true });
            }
            checkEnrollmentStatus(initialCourse.id);
        }
    }, [slug]);

    // ✅ Check if user is enrolled (এটা client-এই থাকবে, localStorage লাগে)
    ...
    const handleLessonSelect = async (lesson) => {
        ...
        try {
            ...
            if (!data.success || !videoUrl) {
                ...
                throw new Error('ভিডিও লিংক পাওয়া যায়নি');
            }
            ...
            setModalUserEmail(data.userEmail || null);
            setIsModalOpen(true);
        } catch (err) {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="relative w-full rounded-xl overflow-hidden bg-black shadow-2xl">
                            <div style={{ position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden' }}>
                                {showIntroVideo && introVideoUrl ? (
                                    ...
                                    <div className="absolute inset-0 cursor-pointer group" onClick={() => { if (introVideoUrl) setShowIntroVideo(true); }}>
                                        {course.thumbnail ? (
                                            <img src={`${API_BASE_URL}/${course.thumbnail}`} alt={course.title} className="absolute inset-0 w-full h-full object-cover" />
                                        ) : (
                                        ...
                                        <div className="absolute bottom-4 left-0 right-0 text-center">
                                            <span className="bg-black/60 text-white text-sm px-4 py-1.5 rounded-full backdrop-blur-sm">কোর্স প্রিভিউ দেখুন</span>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            {course.sections && course.sections.length > 0 ? (
                                course.sections.map((section, sectionIndex) => (
                                    <div key={section.id} className="border-b border-slate-200 last:border-b-0">
                                        <button onClick={() => toggleSection(section.id)} className="w-full flex justify-between items-center py-4 text-left hover:bg-slate-50 px-2 rounded" aria-expanded={openSections[section.id]}>
                                            <h3 className="text-lg font-semibold text-slate-800">Section {sectionIndex + 1}: {section.title}</h3>
                                                        ...
                                                        <li key={lesson.id}>
                                                            ...
                                                            >
                                                                <div className="flex items-center">
                                                                    {isLocked ? <Lock size={16} className="mr-3 text-slate-400" /> : <PlayCircle size={16} className="mr-3 text-green-600" />}
                                ...
                                <div className="flex flex-col md:flex-row gap-6">
                                    <div className="flex flex-col items-center">
                                        <div className="w-32 h-32 rounded-lg overflow-hidden border-2 border-slate-100 mb-3">
                                            <img src={`${API_BASE_URL}/${course.instructor.profilePhoto}`} alt={course.instructor.fullName} className="w-full h-full object-cover" />
                                        </div>
                                    ...
                                    <div className="flex-1">
                                        <h3 className="text-2xl font-bold text-slate-900 mb-2">{course.instructor.fullName}</h3>
                                        ...
                                        <div className="space-y-3 text-slate-700 leading-relaxed">
                                            ...
                                            <p className="text-sm">{course.instructor.detailedBio || "প্রফেশনাল মিডিয়া স্ট্র্যাটেজি এবং প্ল্যানিং বিশেষজ্ঞ।"}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <Link href={`/courses/${relCourse.slug}`} key={relCourse.id} className="group">
                                            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition">
                                                <div className="aspect-video relative">
                                                    <img src={`${API_BASE_URL}/${relCourse.thumbnail}`} alt={relCourse.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                                                </div>
                                                <div className="p-4">
                    ...
                    <aside className="lg:col-span-1 relative">
                        <div className="bg-white rounded-xl shadow-xl border border-slate-100 p-6 sticky top-24 z-10">
                            <div className="mb-6">
                                <p className="text-3xl font-bold text-slate-900 mb-2">{course.isFree ? 'ফ্রি কোর্স' : `৳ ${course.price}`}</p>
                                {!course.isFree && <p className="text-sm text-slate-500 line-through">৳ {Math.round(course.price * 1.5)}</p>}
                            </div>
                            {isEnrolled ? (
                                ...
                                <button onClick={handleEnroll} disabled={enrollmentLoading} className="w-full bg-[#f97316] hover:bg-[#c2570c] text-white font-bold py-3 px-4 rounded-lg mb-4 transition disabled:opacity-50 disabled:cursor-not-allowed">
                                    {enrollmentLoading ? (
                                        <span className="flex items-center justify-center"><span className="animate-spin mr-2">⏳</span>অপেক্ষা করুন...</span>
                                    ) : (
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between"><span className="text-slate-600">মোট লেসন:</span><span className="font-semibold">{course.numberOfLessons || 0}</span></div>
                                <div className="flex justify-between"><span className="text-slate-600">সময়কাল:</span><span className="font-semibold">{course.duration || 'N/A'}</span></div>
                                <div className="flex justify-between"><span className="text-slate-600">ভাষা:</span><span className="font-semibold">{course.language || 'বাংলা'}</span></div>
                            </div>
                        </div>
                    </aside>
                </div>
